...
import { TrendingUp, Briefcase, User, Linkedin, Home, BarChart3, Wrench } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const sections = ['capabilities', 'impact', 'experience', 'skills', 'about'];
  ...
  const navLinks = [
    { name: 'Capabilities', href: '#capabilities', icon: TrendingUp, id: 'capabilities' },
    { name: 'Impact', href: '#impact', icon: BarChart3, id: 'impact' },
  ...
  const mobileNavLinks = [
    ...
    { name: 'GTM', icon: TrendingUp, id: 'capabilities' },
    { name: 'Impact', icon: BarChart3, id: 'impact' },
    { name: 'Work', icon: Briefcase, id: 'experience' },
    { name: 'Connect', icon: Linkedin, id: 'connect' },
  ];
  ...
  const handleScrollTo = (id: string) => {
    if (id === 'home') {
      ...
      window.open(siteContent.footer.linkedin, '_blank');
    } else {
  ...
  // Mobile floating right-side vertical navbar
  if (isMobile) {
    return (
          ...
          <div className="flex items-center space-x-1">
            {navLinks.map((link, index) => {
              ...
              return (
                ...
                >
                  <Icon size={16} className="text-text-secondary group-hover:text-primary transition-colors" />
                  <motion.span
                    initial={{ width: 0, opacity: 0, marginLeft: 0 }}
                    whileHover={{ width: 'auto', opacity: 1, marginLeft: 6 }}
