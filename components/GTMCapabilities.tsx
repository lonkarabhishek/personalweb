...
import { ChevronLeft, TrendingUp, Zap, Cog, BarChart3 } from 'lucide-react';
import { siteContent } from '../content/site';
import { Section, FadeIn } from './ui/Layout';

const icons = [TrendingUp, Zap, Cog, BarChart3];
...
export const GTMCapabilities: React.FC = () => {
  const [cards, setCards] = useState(siteContent.gtmCapabilities.map((s, i) => ({ ...s, id: i })));
  ...
  return (
    <section id="capabilities" className="relative py-12 md:py-24 px-6 md:px-12 lg:px-24" style={{ backgroundColor: '#f5f5f0' }}>
      <div className="max-w-6xl mx-auto">
        <FadeIn>
          ...
          <h2 className="font-sans text-3xl md:text-4xl text-text-primary mb-8 md:mb-16 font-bold tracking-tight">GTM <span className="italic text-[#64748b]">capabilities</span></h2>
          {isMobile && (
      ...
      {isMobile ? (
        <div className="relative h-[560px] flex items-center justify-center">
          ...
          >
            {cards.slice(0, 4).map((capability, index) => {
              const isTop = index === 0;
              ...
              return (
                <DeckCard
                  ...
                  isTop={isTop}
                  onTap={swipeCard}
                  iconColor={iconColors[capability.id % iconColors.length]}
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 flex space-x-2">
            {siteContent.gtmCapabilities.map((_, i) => (
              <div
                key={i}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {siteContent.gtmCapabilities.map((capability, i) => {
            const Icon = icons[i];
            return (
              <FadeIn key={i} delay={i * 0.1} className="h-full">
                <motion.div
                  ...
                  className="bg-white p-8 h-full rounded-2xl border border-gray-200 hover:border-primary/30 hover:shadow-xl transition-all duration-300 flex flex-col group relative overflow-hidden"
                >
                  ...
                  <div className="relative z-10 flex-1">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div
                        className="text-4xl font-extralight text-text-primary/30 tracking-tight"
                        style={{
                    ...
                    <p className="text-[#64748b] leading-[1.8] font-normal mb-6">{capability.description}</p>
                  </div>

                  {/* Tags */}
                  <div className="flex flex-wrap gap-2 relative z-10">
                    {capability.tags.map((tag, j) => (
                      <span
                        key={j}
...
interface DeckCardProps {
  capability: { title: string; description: string; tags: string[]; id: number };
  index: number;
  isTop: boolean;
  onTap: () => void;
  iconColor: string;
...
const DeckCard: React.FC<DeckCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className={`absolute w-full bg-white px-6 py-10 rounded-3xl border border-border shadow-2xl min-h-[500px] ${
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex items-center gap-4 mb-5">
          ...
          >
            {String(capability.id + 1).padStart(2, '0')}.
          </div>
        </div>
        <h3 className="text-2xl md:text-3xl font-sans font-bold text-text-primary mb-4 tracking-tight">
          {capability.title}
        </h3>
        <p className="text-[#64748b] leading-[1.8] text-base font-normal mb-6">{capability.description}</p>
